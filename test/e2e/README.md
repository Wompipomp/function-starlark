...
| **extra-resources-ns** | `composition-extra-resources-ns.yaml` | Namespaced extra resources via `namespace=`: by-name in a namespace, label match scoped to a namespace, label match across all namespaces. Runs only on Crossplane 2.x (the namespace selector is not in the 1.x protocol); skipped on 1.x clusters |
